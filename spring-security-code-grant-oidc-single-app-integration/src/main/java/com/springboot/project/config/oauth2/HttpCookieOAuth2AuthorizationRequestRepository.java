...
public class HttpCookieOAuth2AuthorizationRequestRepository
    ...
    public void saveAuthorizationRequest(
        ...
        if (authorizationRequest == null) {
            CookieProcessor.deleteCookie(
                    request, response, OAUTH2_AUTHORIZATION_REQUEST_COOKIE_NAME);
            CookieProcessor.deleteCookie(request, response, REDIRECT_URI_COOKIE_NAME);
            return;
        }
        ...
        CookieProcessor.addCookie(
                ...
                CookieProcessor.serialize(authorizationRequest),
                cookieExpireSeconds);
        String redirectUriAfterLogin = request.getParameter(REDIRECT_URI_COOKIE_NAME);
        if (StringUtils.isNotBlank(redirectUriAfterLogin)) {
            CookieProcessor.addCookie(
                    response, REDIRECT_URI_COOKIE_NAME, redirectUriAfterLogin, cookieExpireSeconds);
        }
    }
    ...
    public void removeRedirectCookie(HttpServletRequest request, HttpServletResponse response) {
        CookieProcessor.deleteCookie(request, response, REDIRECT_URI_COOKIE_NAME);
    }
}
